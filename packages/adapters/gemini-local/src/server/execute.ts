...
const GEMINI_ROLLOUT_NOISE_RE =
  /^\d{4}-\d{2}-\d{2}T[^\s]+\s+ERROR\s+gemini_core::rollout::list:\s+state db missing rollout path for thread\s+[a-z0-9-]+$/i;

function stripGeminiRolloutNoise(text: string): string {
  ...
  for (const part of parts) {
    ...
    if (GEMINI_ROLLOUT_NOISE_RE.test(trimmed)) continue;
    kept.push(part);
  }
  return kept.join("\n");
}
...
function geminiHomeDir(): string {
  const fromEnv = process.env.GEMINI_HOME;
...
async function ensureGeminiSkillsInjected(onLog: AdapterExecutionContext["onLog"]) {
  const skillsDir = await resolvePaperclipSkillsDir();
  if (!skillsDir) return;

  const skillsHome = path.join(geminiHomeDir(), "skills");
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const buildArgs = (resumeSessionId: string | null) => {
    ...
    if (bypass) {
      ...
      // In a headless environment without bypass, default approval mode will reject tools.
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    const proc = await runChildProcess(runId, command, args, {
      ...
      onLog: async (stream, chunk) => {
        ...
        const cleaned = stripGeminiRolloutNoise(chunk);
        if (!cleaned.trim()) return;
        await onLog(stream, cleaned);
      },
    });
    const cleanedStderr = stripGeminiRolloutNoise(proc.stderr);
    return {
      proc: {
        ...proc,
  ...
  const toResult = (
    attempt: { proc: { exitCode: number | null; signal: string | null; timedOut: boolean; stdout: string; stderr: string }; rawStderr: string; parsed: ReturnType<typeof parseGeminiJsonl> },
